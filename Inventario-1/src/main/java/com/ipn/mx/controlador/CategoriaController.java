...
public class CategoriaController {
	...
	public ResponseEntity<?> read(@Valid @PathVariable Long id){
		...
		try {
			...
			response.put("mensaje", "Error al realizar la consulta en la base de datos".concat(id.toString()).concat(" ERROR INTERNO!!!!"));
			response.put("error", e.getMessage().concat(": ").concat(e.getMostSpecificCause().getMessage()));
			return new ResponseEntity<Map<String,Object>>(response, HttpStatus.INTERNAL_SERVER_ERROR); 
		}
		if(categoria == null) {
			response.put("mensaje", "La Categoria ID: "
	...
	public ResponseEntity<?> update(@Valid @RequestBody Categoria categoria, BindingResult resultado, @PathVariable Long id){
		...
		if(categoriaActual == null) {
			respuesta.put("mensaje", "Error al actualizar la Categoría".concat(id.toString())
	...
	public ResponseEntity<?> delete(@Valid @PathVariable Long id){
		...
		response.put("mensaje", "La categoría se ha eliminado satisfactoriamente");
